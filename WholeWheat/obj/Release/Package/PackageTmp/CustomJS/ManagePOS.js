            ...
            $.ajax({
                ...
                if (data.Success == true) {
                    ...
                    if ($response.pFlag == "1") {
                        ...
                        swal('Failed!', $response.pFlag_Desc, "danger");
                    }
                }
            });
        }
...
$(document).on('click', '.add_to_cart', function (event) {
    ...
    if (RowExist === 1) {
        ...
        $(CurrentRow).closest('tr').removeClass('bg-primary');
...
$(document).on("click", ".incbutton", function () {
    ...
    var inc = items + 1;
    $('#ItemsNum span,#ItemsNum2 span').text(parseInt(inc));
    $(this).closest('tr').removeClass('bg-primary');
...
function saleBtn() {
    ...
    else if ($('#ddlsaletype').val() == "3" && $('.Delivery').val() == 0) {
